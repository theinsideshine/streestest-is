...
def send_request(url, thread_num):
    try:
        ...
        formatted_start_time = start_time.strftime("%H-%M-%S.%f")[:-3]  # Formato hora, minutos, segundos y milisegundos
        ...
        with print_lock:
            if response.status_code == 200:
                print(f"Hilo {thread_num}: Hora de solicitud: {formatted_start_time} Tiempo de respuesta: {response_time} segundos")
            else:
                print(f"Hilo {thread_num}: Hora de solicitud: {formatted_start_time} Tiempo de respuesta: -1 segundos")
...
def send_requests_concurrently(num_requests, urls , time_delay_s):
    ...
    for i in range(num_requests):
        ...
        thread = threading.Thread(target=send_request, args=(url, i+1))  # Pasar el número de hilo como argumento
        threads.append(thread)
        thread.start()
        time.sleep(time_delay_s)  # Espera time_delay_s segundo antes de iniciar el próximo hilo
    for thread in threads:
        thread.join()

# URLs de los endpoints
urls = ["http://localhost:8081/tracks/mp3/2",
        "http://localhost:8081/tracks/mp3/3"]
...
file_name = f"localRDS-Nr{num_requests}-Td{time_delay_ms}-Ne{error_count}.csv"
